class WormController {
    ...
    availableDashCharge = 150;
    maxDashCharge = 150;

    continualGravityAffect = 0;
    ...
    applyAcceleration(mousePosition, mouseDown, bodyShape) {
        ...
        if (undergroundStrength === 1) {
            this.continualGravityAffect = 0;
        } else {
            accelVec.mul(undergroundStrength / 2 + 0.5);

            this.continualGravityAffect += 0.01 * (1 - undergroundStrength);
            this.continualGravityAffect -= this.continualGravityAffect * this.continualGravityAffect * 0.1;

            accelVec.y += this.continualGravityAffect;
        }

        if (!mouseDown) {
            this.availableDashCharge = Math.min(this.availableDashCharge + undergroundStrength * 0.5, this.maxDashCharge);
        }
