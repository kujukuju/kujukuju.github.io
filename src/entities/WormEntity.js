class WormEntity {
    ...
    constructor(username) {
        ...
        this.maxSegmentLength = WormEntity.TEXTURE.width / 20;
        this.points = [];
        for (let i = 0; i <= 20; i++) {
            this.points.push(new PIXI.Point(0, 0));
        }

        this.sprite = new PIXI.SimpleRope(WormEntity.TEXTURE, this.points);

        this.controller = new WormController();
        ...
        if (username === 'bald') {
            this.maxHearts = WormEntity.maxHearts;
            this.remainingHearts = WormEntity.remainingHearts;
        } else {
            this.maxHearts = 6;
            this.remainingHearts = 6;
        }
        ...
        this.audio = WormEntity.AUDIO.create();
    ...
    update() {
        ...
        if (EntityInformation.getClientEntity() === this) {
            ...
            for (let i = 0; i <= 20; i++) {
                const distance = WormEntity.TEXTURE.width - i * this.maxSegmentLength;
            ...
            this.name.position.y = this.controller.position.y - 20;
            ...
            this.name.position.y = position.y - 18;

            this.hearts.update(this);
            this.hearts.position.x = position.x;
            this.hearts.position.y = position.y - 28;
    ...
    sendPackets() {
        ...
        index = Packets.writeEntityPacket(PacketProcessor.ENTITY_TYPE_WORM, bytes, index);
    ...
    kill() {
        ...
        DeadBodyManager.addDeadWormEntity(this.points);
        EntityInformation.addDeadEntity(this.__name);

        if (EntityInformation.getClientEntity() === this) {
            GameState.spawnCooldown = GameState.DEATH_COOLDOWN;
        } else {
            SoulPlantManager.receiveSoul(this.points[this.points.length - 1], WormEntity);
        }
